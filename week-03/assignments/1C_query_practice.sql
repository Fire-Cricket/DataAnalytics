...
-- Write a query against the orders table that displays the total
...
-- for each country where orders have been shipped. Again, use an alias to label the count as CustomerCount. 
...
-- What are the products that we carry where we have less than 25 units on hand, 
-- but 1 or more units of them are on order? Write a query that orders them 
-- by quantity on order (high to low), then by product name.
...
-- What employees have a monthly salary 
-- that is between $2000 and $2500? Write a query that orders them by job title.
